...
@mock.patch("scripts.run_smoke_test.AlertManager")  # Added AlertManager patch
...
def test_run_smoke_test_successful_flow(
    mock_alert_manager_class,  # Added mock_alert_manager_class
    mock_ad_sentinel_class,
    mock_get_metrics,
    ...
    mock_am_instance = (
        mock_alert_manager_class.return_value
    )  # Get instance of mocked AM
    ...
    args, kwargs = mock_ad_sentinel_class.call_args
    assert kwargs["max_budget"] == float(budget_to_test)
    assert kwargs["campaign_id"] == mock_campaign_id_val
    assert "halt_callback" in kwargs
    assert "alert_manager" in kwargs  # Check for new alert_manager arg
    assert (
        kwargs["alert_manager"] == mock_am_instance
...
def test_run_smoke_test_ctr_below_target(
    ...
    mock_am_instance = mock_alert_manager_class.return_value  # Get instance
    ...
    # Verify AdBudgetSentinel was instantiated with AlertManager
    mock_ad_sentinel_class.assert_called_once()
    args, kwargs = mock_ad_sentinel_class.call_args
    assert "alert_manager" in kwargs
    assert kwargs["alert_manager"] == mock_am_instance
...
def test_run_smoke_test_ad_budget_exceeded(
    ...
    assert kwargs["max_budget"] == budget
    assert kwargs["campaign_id"] == mock_campaign_id
    assert "alert_manager" in kwargs  # Check for new alert_manager arg
    assert (
        kwargs["alert_manager"] == mock_am_instance
    )  # Check it's the correct instance
