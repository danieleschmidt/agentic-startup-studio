...
@mock.patch(
    "scripts.run_smoke_test.SMOKE_TEST_RESULTS_DIR", str(TEST_OUTPUT_BASE_DIR)
)
def test_run_smoke_test_successful_flow(
    ...
    result = runner.invoke(
        run_smoke_test,
        [
            "--idea-id", idea_id_to_test,
            "--budget", budget_to_test,
            "--results-dir", str(TEST_OUTPUT_BASE_DIR),
        ],
    )
    ...
    step1_msg = (
        f"Step 1: Fetching details for Idea ID: {idea_id_to_test} (Placeholder)"
    )
