...
def test_check_usage_over_budget_with_alert_manager(mock_alert_manager: MagicMock):
    ...
    # Message recorded by AlertManager/internally includes "ALERT: "
    ...
    mock_alert_manager.record_alert.assert_called_once_with(
        message=expected_alert_message_with_prefix,
        level="CRITICAL",
...
def test_check_usage_over_budget_no_alert_manager(capsys):
    ...
    expected_warning_content = (
        f"Token budget exceeded in context '{context}'. Usage: 70, Budget: 50."
    )
    # The fallback print in the sentinel prepends "ALERT: "
...
def test_get_and_clear_internal_alerts():
...
def test_alert_manager_call_exception_fallback_print(
    ...
    captured = capsys.readouterr()
    expected_error_msg = (
        "Error using AlertManager from TokenBudgetSentinel: AlertManager failed!"
    )
    ...
    # Corrected E501
    fallback_msg = (
        "Warning (TokenBudgetSentinel Fallback): ALERT: "
        f"{expected_fallback_log_content}"
    )
