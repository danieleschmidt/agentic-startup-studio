...
from .config import ValidationEngineConfig, ValidationArea
...
class ValidationEngine:
    ...
    def _discover_validator_classes(self, plugin_dir: Path) -> List[Type[BaseValidator]]:
        ...
        try:
            ...
            if hasattr(validator_class, '__init__'):
                ...
                if len(params) == 0:
                    ...
                    validator = validator_class(self.config, ValidationArea.DATA_FLOW, "test")
                else:
            ...
            logger.info(f"Registered validator: {validator_class.__name__} for area {area}")
        except Exception as e:
            logger.error(f"Failed to register validator {validator_class.__name__}: {e}")
            raise
        ...
        **kwargs
        ...
        if len(results) == 1:
            ...
            return ValidationResult(
                ...
                area=area,
                passed=all_passed,
                message="; ".join(messages) if messages else "Multiple validations executed",
            ...
            try:
                if hasattr(validator, 'validate_async'):
                    ...
                    result = await loop.run_in_executor(
                        None, 
                        lambda: validator.validate(data, **kwargs)
                    )
                return result
    ...
    def get_validation_summary(self, results: Dict[ValidationArea, List[ValidationResult]]) -> Dict[str, Any]:
