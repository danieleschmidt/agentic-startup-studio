...
def init_db() -> None:
    ...
    try:
        ...
        raise CLIError(f"Failed to initialize database: {exc}") from exc
...
def display_idea_summary(summary: IdeaSummary) -> None:
    ...
    progress_bar = "█" * int(summary.progress * 20) + "░" * (
        20 - int(summary.progress * 20)
    )
...
def validate_idea_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "evidence_links" in data and isinstance(data["evidence_links"], str):
        ...
        links = [
            link.strip() for link in data["evidence_links"].split(",") if link.strip()
        ]
...
def cli(ctx, verbose):
    ...
    # Initialize database lazily to avoid import-time side effects
    init_db()
...
@cli.command()
@click.option(
    "--title", prompt="Idea title", help="Short, descriptive title for the idea"
)
@click.option(
    "--description",
    prompt="Idea description",
    help="Detailed description of the startup idea",
)
@click.option(
    "--category",
    type=click.Choice([cat.value for cat in IdeaCategory]),
...
@click.option(
    "--force", is_flag=True, help="Force creation even if similar ideas exist"
)
@click.option(
    "--output",
...
@handle_async
async def create(
    title, description, category, problem, solution, market, evidence, force, output
):
...
@click.option(
    "--status",
    type=click.Choice([s.value for s in IdeaStatus]),
...
@click.option(
    "--category",
    type=click.Choice([c.value for c in IdeaCategory]),
...
async def update(idea_id, title, description, category, problem, solution, market):
    ...
    try:
        ...
        if category:
            updates["category"] = IdeaCategory(category)
        if problem:
            updates["problem_statement"] = problem
        if solution:
            updates["solution_description"] = solution
        if market:
            updates["target_market"] = market
        ...
        with console.status("[bold green]Updating idea..."):
            success = await manager.update_idea(
                idea_id=uuid_obj, updates=updates, user_id="cli_user"
            )
    ...
    try:
        ...
        with console.status(f"[bold green]Advancing to {stage}..."):
            success = await manager.advance_stage(
                idea_id=uuid_obj, next_stage=target_stage, user_id="cli_user"
            )
...
class IdeaIngestionCLI:
    ...
    async def create_idea(
        ...
        try:
            ...
            idea_id, warnings = await self.idea_manager.create_idea(
                raw_data=idea_data, force_create=force, user_id=user_id or "cli_user"
            )
    ...
    async def list_ideas(
        ...
        try:
            ...
            filters = QueryParams(
                ...
                category_filter=(
                    [self._parse_category_name(category)] if category else None
                ),
                limit=limit,
            )
    ...
    async def advance_stage(
        ...
        try:
            ...
            success = await self.idea_manager.advance_stage(
                idea_id=uuid_obj, next_stage=stage_obj, user_id=user_id or "cli_user"
            )
    ...
    def _format_idea_details(self, idea: Idea) -> str:
        ...
        stage_value = (
            ...
            if hasattr(idea.current_stage, "value")
            else str(idea.current_stage)
        )
...
def main() -> None:
    """Entry point for the ingestion CLI used in tests."""
    import argparse

    parser = argparse.ArgumentParser(description="Idea ingestion CLI")
    ...
    adv_p = subparsers.add_parser("advance")
    adv_p.add_argument("--id", required=True)
    adv_p.add_argument("--stage", required=True)

    args = parser.parse_args()

    cli = asyncio.run(create_cli_interface())
    ...
    if args.command == "create":
        ...
        asyncio.run(cli.get_idea(args.id))
    elif args.command == "list":
        asyncio.run(cli.list_ideas(limit=args.limit))
    elif args.command == "advance":
        asyncio.run(cli.advance_stage(idea_id=args.id, next_stage=args.stage))
