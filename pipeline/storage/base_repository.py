...
from pipeline.utils.backoff import exponential_backoff
...
class BaseRepository(Generic[T], ABC):
    ...
    ) -> Union[List[Record], Record, Any, None]:
        """
    ...
    async def find_by_id(self, entity_id: UUID) -> Optional[T]:
        ...
        if record:
            ...
            # Cache result
            self._query_cache[cache_key] = (entity, datetime.now(timezone.utc))
            return entity
...
class CachedRepository(BaseRepository[T]):
    ...
    async def find_by_id(self, entity_id: UUID) -> Optional[T]:
        ...
        if entity:
            self._entity_cache[cache_key] = (entity, datetime.now(timezone.utc))
