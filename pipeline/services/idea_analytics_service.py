...
class MarketPotentialScore(BaseModel):
    ...
    market_size_estimate: Optional[float] = None  # USD millions
    competition_density: Optional[float] = None  # 0.0 to 1.0
...
class IdeaAnalyticsService:
    ...
    def _recommend_funding_stage(self, idea: Idea, funding_score: float) -> str:
        ...
        if idea.current_stage in [PipelineStage.IDEATE, PipelineStage.RESEARCH]:
            return "pre_seed"
        elif idea.current_stage in [PipelineStage.DECK, PipelineStage.INVESTORS]:
            return "seed" if funding_score > 0.6 else "pre_seed"
        elif idea.current_stage in [PipelineStage.MVP, PipelineStage.BUILDING]:
