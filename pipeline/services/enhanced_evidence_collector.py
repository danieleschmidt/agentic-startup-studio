...
class EnhancedEvidenceCollector:
    ...
    def _classify_source_type(self, url: str) -> str:
        ...
        if any(domain in url_lower for domain in [".edu", "scholar.google", "arxiv", "pubmed"]):
            return "academic"
        if any(domain in url_lower for domain in ["mckinsey", "bcg", "deloitte", "pwc"]):
            return "industry_report"
        if any(domain in url_lower for domain in [".gov", "europa.eu", "oecd"]):
            return "government"
        if any(domain in url_lower for domain in ["reuters", "bloomberg", "techcrunch", "cnn"]):
            return "news"
        if "patent" in url_lower:
            return "patent"
        if any(domain in url_lower for domain in ["medium.com", "wordpress", "blogspot"]):
            return "blog"
        return "company"
    ...
    def _extract_publish_date(self, result: dict[str, Any]) -> datetime | None:
