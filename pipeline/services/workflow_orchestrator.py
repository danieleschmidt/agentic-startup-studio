...
class WorkflowOrchestrator:
    ...
    async def resume_workflow(self, idea_id: str) -> WorkflowState | None:
    ...
    async def _process_deck_generation_stage(self, state: WorkflowState):
        """Generate a simple pitch deck."""
        idea = state.get("idea_data", {})
        deck_data = {
            "title": idea.get("title", "Startup"),
            "subtitle": idea.get("description", "")[:60],
            "sections": [
                {"title": "Problem", "content": idea.get("problem_statement", "")},
                {"title": "Solution", "content": idea.get("solution_description", "")},
                {"title": "Market", "content": idea.get("target_market", "")},
            ],
        }
        template_path = self.settings.deck.template_path
        deck_md = generate_deck_content(deck_data, template_path)
        state["deck_data"] = {
            "deck_content": deck_md,
            "slide_count": deck_md.count("---") - 1,
            "accessibility_score": 95,
        }
        await asyncio.sleep(0.1)

    async def _process_investor_evaluation_stage(self, state: WorkflowState):
        """Score the deck using investor rubric."""
        deck = state.get("deck_data", {}).get("deck_content", "")
        idea = state.get("idea_data", {})
        profile_path = self.settings.investor.get_profile_path()
        try:
            profile = load_investor_profile(profile_path)
            score, feedback = score_pitch_with_rubric(
                deck,
                {
                    "idea_name": idea.get("title", "idea"),
                    "idea_description": idea.get("description", ""),
                },
                profile["scoring_rubric"],
            )
        except Exception as e:
            self.logger.error(f"Investor evaluation failed: {e}")
            score, feedback = 0.0, [f"Error: {e}"]

        state["investor_data"] = {"investor_score": score, "feedback": feedback}
        await asyncio.sleep(0.1)
    ...
    async def _validate_investor_quality_gate(self, state: WorkflowState) -> QualityGateResult:
        ...
        funding_threshold = self.settings.budget.funding_threshold
