...
class PitchDeckGenerator:
    ...
    def __init__(self):
        ...
        if not api_key:
            self.logger.warning("OpenAI API key not configured - pitch deck generation will fail")
            self.llm = None
        else:
            self.llm = ChatOpenAI(
                api_key=api_key,
    ...
    def _build_workflow(self) -> Graph:
        ...
        workflow.add_node("generate_business_model_slide", self._generate_business_model_slide)
    ...
    async def _validate_quality_gates(self, inputs: Dict) -> Dict:
        ...
        low_quality_slides = [
            slide for slide in state.generated_slides
        ...
        total_evidence = sum(len(slide.supporting_evidence) for slide in state.generated_slides)
    ...
    async def _analyze_evidence_insights(self, evidence_by_domain: Dict[str, List[Evidence]]) -> Dict:
        """Analyze evidence for key insights."""
