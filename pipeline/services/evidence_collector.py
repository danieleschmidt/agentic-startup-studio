    ...
    async def collect_evidence(
        ...
        try:
            ...
            async with self.budget_sentinel.track_operation(
                ...
                for domain, result in zip(research_domains, domain_results, strict=False):
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
                async with session.head(url) as response:
                    ...
                    if response.status == 200:
                        return 1.0
                    if response.status in [301, 302, 303, 307, 308]:  # Redirects
                        return 0.8
                    if response.status == 403:  # Forbidden but exists
                        return 0.3
                    return 0.0
